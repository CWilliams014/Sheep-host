...
describe('ApiSandBox />', function() {
	xit('should render 4 input fields', function() {
		const wrapper = shallow(<ApiSandBox />)
		expect(wrapper.find('input')).to.have.length(4)
	})
	xit('should render div with className = display jumbotron', function() {
		const wrapper = shallow(<ApiSandBox />)
...
describe('DataItem />', function() {
	xit('should have props = info', function() {
		const wrapper = shallow(<DataItem />)
		expect(wrapper.props().info).to.be.defined
	})
	xit('should render a list item with className = i list-group-item', function() {
		const info = [3,4,5,6]
...
describe('DBNavBar />', () => {
	...
	xit('should have props = names', () => {
		const wrapper = shallow(<DBNavBar names={names}/>)
		expect(wrapper.props().names).to.be.defined
	})
	xit('should return proper amount of button tabs', () => {
		const wrapper = shallow(<DBNavBar names={names}/>)
